...
restartButton.addEventListener('click', () => {
  ...
  // Reset game state to Level 1 and start again
  startGame(true); // Pass true to indicate restart
  updateLeaderboard();
});
...
function startGame(isRestart = false) {
  ...
  // Reset timer if it's a restart
  if (isRestart) {
    resetTimer();
  }

  // Reset the leaderboard for a restart if needed
  if (!isRestart) {
    addToLeaderboard(level, timeLeft);
  }
}
...
// Initialize the game
startGame(); // Starts the game for the first time
